      ...
      <div className={`max-w-[1400px] w-full flex ${isMobile && "items-center"} flex-col gap-5 mx-5 xxl:mx-0`}>
        ...
        <Suspense fallback={<div className="h-16 bg-[#f9f4f4] animate-pulse"></div>}>
          ...
          handleClear={()=>setFilters((prevForm)=> ({...prevForm,location:[]}))}
            filters={filters}
            setCity={(e) => { e && setFilters((prevForm: IFilter) => ({ ...prevForm, city: e ,location:[]})) } }
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {isMobile && (
              <button
                ...
                className="relative flex items-center text-sm bg-white rounded-lg p-2 gap-1"
              >
                <CiFilter />
                {filters.features.length > 0 && (
                  <span className="w-full h-3 bg-[#1f1f29] rounded-full absolute right-[-5px] top-[-5px]"></span>
                )}
                Filters
              </button>
            )}
