...
const ManageProperties: FC<Props> = ({ user, setRoute, handleEditListing }) => {
  ...
  return (
    <>
      <table className="text-left w-full capitalize text-xs h-full">
        <thead className="bg-[#1f1f29] text-white font-normal">
          <tr>
              ...
              <tr key={index} className="border-b">
                ...
                <td className="p-4 relative text-xl text-zinc-400 ">
                  ...
                  {actionOpen === property._id && (
                    <ul className="dropdown z-30 top-2 right-20 bg-white absolute py-2 rounded-md text-base leading-7 shadow-xl text-xs  border text-[#1f1f29]">
                      <Link href={`/property/${property._id}`}>
                        <li className="px-4 flex gap-2 items-center cursor-pointer">
                          <IoEyeOutline />
                          View
                        </li>
                      </Link>
