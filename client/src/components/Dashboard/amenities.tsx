...
import AmenitiesSkeleton from "../Skeletons/AmenitiesSkeleton";

const Feature = lazy(() => import("./features"));
  ...
  return (
    <div className="container mx-auto p-5 text-center w-auto h-auto">
      ...
      <div className="h-full max-h-[140px] xs:max-h-[70px] overflow-auto max-w-[400px]">
        {!filter && removeTags && (
          <TagList tag={allTags as Tag[]} handleRemoveTag={removeTags} />
        )}
      </div>
        ...
        <Suspense fallback={<AmenitiesSkeleton />}>
          {searchResults.map((item) => (
            <Feature
              ...
              activeFeature={
                filter ? (allTags as string[]) : (allTags as Tag[])
              }
              filter={filter}
            />
          ))}
